...
const ProductCard = memo(({ product }) => {
  return (
    <article
      ...
      className="product-card relative flex-shrink-0 w-[80vw] sm:w-[45vw] md:w-[35vw] lg:w-[28vw] h-[60vh] sm:h-[65vh] md:h-[70vh] rounded-[2.5rem] overflow-hidden group mr-6 sm:mr-8 md:mr-10 select-none isolate bg-surface-container"
      style={{
        minWidth: '280px',
        minHeight: '320px',
      }}
    >
      <img
        ...
        width={600}
        height={800}
        className="w-full h-full object-cover transition-[filter,transform] duration-700 brightness-[0.85] group-hover:brightness-110 group-hover:scale-105 will-change-transform block"
        loading="lazy"
        decoding="async"
      />
      <div
        aria-hidden="true"
        className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent pointer-events-none select-none z-10 transition-opacity duration-500 group-hover:opacity-80"
      />
      <div className="absolute bottom-0 left-0 w-full p-8 md:p-10 flex flex-col justify-end pointer-events-none z-20">
        <h3 className="font-headline text-2xl md:text-3xl font-black text-white tracking-tight uppercase mb-1 drop-shadow-lg transition-transform duration-500 group-hover:-translate-y-1">
          {product.name}
        </h3>
        <div className="flex justify-between items-end drop-shadow-md">
          ...
          <span className="font-headline text-lg md:text-xl font-bold text-white text-shadow-sm">
            {product.price}
          </span>
        </div>
      </div>
...
const Products = () => {
  return (
    ...
    >
      ...
      <div className="px-6 sm:px-12 md:px-20 mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6 relative z-30">
        <div>
          <span className="text-primary font-label text-sm font-bold tracking-[0.3em] uppercase mb-3 block">
            Most Popular
          </span>
          <h2 className="font-headline text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-on-surface tracking-tighter leading-[0.9]">
            INFINITE <br />
            <span className="text-surface-container-highest flex items-center gap-4">
              FLAVORS
              <div aria-hidden="true" className="h-1 w-12 sm:w-20 md:w-40 bg-primary/20 rounded-full hidden sm:block" />
            </span>
          </h2>
        </div>
      ...
      <div
        aria-hidden="true"
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[30vw] font-black text-on-surface/[0.03] select-none pointer-events-none z-0 whitespace-nowrap"
      >
        POPULAR COLLECTION
      </div>
    </section>
